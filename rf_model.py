"""Random Forest vs LightGBM comparison on the same train/test split."""
...
log_preds_rf = rf.predict(X_test)
...
SEP = "-" * 62
print(f"\n{SEP}")
print(f"  {'Metric':<24}  {'Random Forest':>15}  {'LightGBM':>15}")
print(SEP)
rows = [
    ("R2 (log)",     ".4f"),
    ("R2 (EUR)",     ".4f"),
...
print(f"  Random Forest closer: {(lgbm_err >= rf_err).mean()*100:.1f}% of transfers")
