"""SVM (SVR) vs LightGBM comparison on the same train/test split."""
...
# ── SVM: scale features (mandatory for SVR) ───────────────────────────────────
scaler  = StandardScaler()
X_train_s = scaler.fit_transform(X_train)
X_test_s  = scaler.transform(X_test)

print("Training SVR (RBF kernel, C=10, eps=0.1)...")
svr = SVR(kernel="rbf", C=10, epsilon=0.1, gamma="scale")
svr.fit(X_train_s, y_train)
print("  Done.")

log_preds_svm = svr.predict(X_test_s)
y_pred_svm    = np.expm1(log_preds_svm)
y_true        = np.expm1(y_test)

# ── LightGBM: train inline for fair comparison ────────────────────────────────
...
svm_stats  = summarise(y_true, y_pred_svm,  y_test, log_preds_svm)
lgbm_stats = summarise(y_true, y_pred_lgbm, y_test, log_preds_lgbm)

# ── overall table ─────────────────────────────────────────────────────────────
SEP = "-" * 62
print(f"\n{SEP}")
print(f"  {'Metric':<24}  {'SVR (RBF)':>15}  {'LightGBM':>15}")
print(SEP)
rows = [
    ("R2 (log)",     ".4f"),
    ("R2 (EUR)",     ".4f"),
...
for name, fmt in rows:
    a, b = svm_stats[name], lgbm_stats[name]
    if fmt == "pct":
...
svm_err  = np.abs(y_pred_svm  - y_true)
lgbm_err = np.abs(y_pred_lgbm - y_true)
print(f"\n  LightGBM closer: {(lgbm_err < svm_err).mean()*100:.1f}% of transfers")
print(f"  SVR closer:      {(lgbm_err >= svm_err).mean()*100:.1f}% of transfers")

# ── by market value range ──────────────────────────────────────────────────────
y_mv     = df_test["market_value_in_eur"].values
svm_pct  = svm_err  / y_true * 100
...
print(f"  {'MV Range':<8}  {'n':>5}  {'SVR':>8}  {'LightGBM':>9}  {'LGBM wins':>9}")
for b in labels:
    mask = mv_band == b
    if not mask.any():
        continue
    lgbm_wins = (lgbm_err[mask] < svm_err[mask]).mean() * 100
    print(f"  {b:<8}  {mask.sum():>5}  "
          f"{np.median(svm_pct[mask]):>7.1f}%  "
