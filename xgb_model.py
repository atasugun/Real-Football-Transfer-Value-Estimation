"""XGBoost vs LightGBM comparison on the same train/test split."""
...
X_tr, X_val, y_tr, y_val = train_test_split(
    df_train[feature_cols], df_train["log_fee"], test_size=0.12, random_state=0
)
...
print(f"  Best n_estimators: {xgb_model.best_iteration}")

log_preds_xgb = xgb_model.predict(X_test)
y_pred_xgb    = np.expm1(log_preds_xgb)
...
log_preds_lgbm = lgbm.predict(X_test)
...
xgb_stats  = summarise(y_true, y_pred_xgb,  y_test, log_preds_xgb)
lgbm_stats = summarise(y_true, y_pred_lgbm, y_test, log_preds_lgbm)

# ── overall table ─────────────────────────────────────────────────────────────
SEP = "-" * 62
print(f"\n{SEP}")
print(f"  {'Metric':<24}  {'XGBoost':>15}  {'LightGBM':>15}")
print(SEP)
rows = [
    ("R2 (log)",     ".4f"),
    ("R2 (EUR)",     ".4f"),
...
for name, fmt in rows:
    a, b = xgb_stats[name], lgbm_stats[name]
    if fmt == "pct":
...
xgb_err  = np.abs(y_pred_xgb  - y_true)
lgbm_err = np.abs(y_pred_lgbm - y_true)
print(f"\n  LightGBM closer:  {(lgbm_err < xgb_err).mean()*100:.1f}% of transfers")
print(f"  XGBoost closer:   {(lgbm_err >= xgb_err).mean()*100:.1f}% of transfers")

# ── by market value range ──────────────────────────────────────────────────────
y_mv     = df_test["market_value_in_eur"].values
xgb_pct  = xgb_err  / y_true * 100
...
print(f"  {'MV Range':<8}  {'n':>5}  {'XGB':>8}  {'LightGBM':>9}  {'LGBM wins':>9}")
for b in labels:
    mask = mv_band == b
    if not mask.any():
        continue
    lgbm_wins = (lgbm_err[mask] < xgb_err[mask]).mean() * 100
    print(f"  {b:<8}  {mask.sum():>5}  "
          f"{np.median(xgb_pct[mask]):>7.1f}%  "
